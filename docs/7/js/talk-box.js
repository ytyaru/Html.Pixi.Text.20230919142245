...
class TalkBox extends PIXI.Container {
    constructor(text='', style={fontFamily:'IPAexゴシック', fontSize:24, letterSpacing:24*0.05, lineHeight:24*1.70, fill:0xFF0000, align:'left', stroke:'#FFFF00', strokeThickness:4}, frameId=0) {
        ...
        //this._frame.draw(0)
        //gsap.to(this._frame, { _appearWidth:this._frame.Width });
        //gsap.to(this._frame, { AppearWidth:this._frame.Width });
    }
...
//        this._frame.draw(delta)
        ...
        if (this._frame.appear()) { this._text.draw(delta) }
    }
    #setPos() {
...
class TalkBalloon extends PIXI.Graphics {
    ...
    appear() {
        ...
        this.drawPolygon([this._x,63, 
            this._x + ((this._appearWidth < this._tipWidth) ? this._appearWidth : this._tipWidth),50, 
            this._x + ((this._appearWidth < this._tipWidth) ? this._appearWidth : this._tipWidth),75]);
        this.endFill();
        //if (this._appearWidth < this._tipWidth) { return false }
        // 枠
        this.beginFill(0x22aa22, 0.3); // color, alpha
    ...
    */
    ...
    get AppearWidth() { return this._appearWidth }
    ...
    set X(v) { this._x = v }
    set Y(v) { this._y = v }
    set AppearWidth(v) { this._appearWidth = v }
}
